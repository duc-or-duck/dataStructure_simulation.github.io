let listt = document.getElementById('main__linkedList-list');
let nodes = document.getElementsByClassName('node');
let pointers = document.getElementsByClassName('pointer');
let error = document.getElementById('main__linkedList-error');
...
function deleteNode(i) {
    return new Promise(resolve => {
        ...
        setTimeout(() => {
            listt.removeChild(nodes[i]);
            listt.removeChild(pointers[i]);
            resolve();
        }, deleteTimeout);
    });
}
...
async function add(i, data) {
    ...
    if (i === nodes.length) {
        ...
        listt.appendChild(pointer);
    } else {
        await animateNodes(0, i - 1);
        await animateNodesBeforeInsert(i, nodes.length)
        listt.insertBefore(pointer, nodes[i]);
        listt.insertBefore(node, pointer);
    }
